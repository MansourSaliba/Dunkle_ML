"""
Phase I - Sparse Regression (STLSQ) Trial 3

Goal:
- Keep the same STLSQ workflow as Trial 2.
- Enrich candidate libraries with:
  1) all second-order product combinations,
    2) real cube-root transformed combinations (power 1/3) for base/second terms.

Data split:
- Sequential 70% train / 20% val / 10% test.

Outputs (trial 3 specific):
- sindy_trial3_selection_hc.csv
- sindy_trial3_selection_he.csv
- sindy_trial3_coefficients_hc.csv
- sindy_trial3_coefficients_he.csv
- sindy_trial3_loss_progress_hc.png
- sindy_trial3_loss_progress_he.png
- sindy_trial3_forward_predictions_hc.csv
- sindy_trial3_forward_predictions_he.csv
- sindy_trial3_forward_overlay_hc.png
- sindy_trial3_forward_overlay_he.png
- phase1_sindy_trial3_summary.csv
"""
...
MAX_STLSQ_ITERS = 50
RIDGE = 1e-10
THRESHOLD_MIN_SCALE = 0.02
THRESHOLD_MAX_SCALE = 0.1
VAL_SPARSITY_WINDOW = 0.05
...
def build_expanded_terms(base: dict[str, np.ndarray]) -> tuple[np.ndarray, list[str]]:
    """
    Construct features using:
    - base terms,
    - all second-order products,
    - real cube-root transforms of base/second terms.
    """
    ...
    # Cube-root transformed combinations (physics-inspired 1/3 power)
    # We apply cbrt to each constructed term above.
    transformed = {f"cbrt({k})": real_cuberoot(v) for k, v in terms.items()}
    terms.update(transformed)

    feature_names = ["1"] + list(terms.keys())
    X = np.column_stack([np.ones(len(next(iter(base.values()))), dtype=float)] + [terms[k] for k in terms])
...
def build_library_hc(df: pd.DataFrame) -> tuple[np.ndarray, list[str]]:
    ...
    return build_expanded_terms(base)
...
def build_library_he(df: pd.DataFrame) -> tuple[np.ndarray, list[str]]:
    ...
    # Trial 3 keeps he library independent from hc as in Trial 2.
    base = {
        "Tw": Tw,
        "Tgi": Tgi,
        "Pw": Pw,
...
def run_sparse_regression(
    ...
    coeff_path = out_dir / f"sindy_trial3_coefficients_{name}.csv"
    selection_path = out_dir / f"sindy_trial3_selection_{name}.csv"
    coeff_df.to_csv(coeff_path, index=False)
    selection_df.to_csv(selection_path, index=False)

    plot_path = out_dir / f"sindy_trial3_loss_progress_{name}.png"
    ...
    plt.ylabel("MSE")
    plt.title(f"STLSQ Trial 3 Loss Progression - {name}")
...
def evaluate_forward_predictions(
    ...
    pred_path = out_dir / f"sindy_trial3_forward_predictions_{target_name}.csv"
    pred_df.to_csv(pred_path, index=False)

    plot_path = out_dir / f"sindy_trial3_forward_overlay_{target_name}.png"
    ...
    plt.title(f"SINDy Trial 3 Forward Comparison - {target_name}")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(plot_path, dpi=150, bbox_inches="tight")
...
def main() -> None:
    ...
    print("Running STLSQ sparse regression Trial 3 for hc and he...")
    ...
    summary_path = base_dir / "phase1_sindy_trial3_summary.csv"
    summary_df.to_csv(summary_path, index=False)

    print("=" * 72)
    print("Sparse regression Trial 3 complete.")
